...
def add_user(request):
    try:
        ...
        return render(request, 'orders/error.html', {'error': 'Something went wrong!'})
    return render(request, 'orders/login.html', {'message': 'Registration complete'})
...
def create_pizza(request):
    ...
    if request.POST['is_special'] == 'True':
        return render(request, 'orders/add_special.html', context)
        ...
        return render(request, 'orders/add_toppings.html', context)
...
def create_sub(request):
    context = {
        ...
        'name': request.POST['name'],
        'size': request.POST['size'],
        'additions': Sub.objects.get(name=request.POST['name']).possible_additions.all()
    }
    return render(request, 'orders/add_sub.html', context)

def add_sub(request):
    additions=[]
    for addition in Sub.objects.get(name=request.POST['name']).possible_additions.all():
